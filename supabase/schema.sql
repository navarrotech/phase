...
-- ---------------------------------------------------------------------------
-- LLM opponent credentials.
--
-- Holds one row per (account, provider) so a player can face a reasoning model
-- instead of the built-in AI. The credential is the PLAYER'S own — the project
-- never proxies games onto a maintainer's account — and it is stored SEALED,
-- never in plaintext.
--
-- Sealing happens in the lobby Worker (POST /llm/credential) under AES-256-GCM
-- with a key held only as a Worker secret. This table therefore holds
-- ciphertext that is inert on its own: reading every row here, with the anon
-- key or a full dump, yields no usable API key. The Worker unseals in memory
-- for the duration of one upstream call and persists nothing.
--
-- These rows are deliberately OUTSIDE the `user_backups` envelope. That
-- envelope is a verbatim snapshot of user-owned localStorage which the client
-- also writes to disk on "export backup", so anything reachable from it lands
-- in a plaintext file the user may share. A credential must never travel that
-- path.
create table if not exists public.llm_credentials (
  user_id     uuid        not null references auth.users (id) on delete cascade,
  -- Matches the Worker's provider union. A check constraint rather than an
  -- enum type so adding a provider is a one-line migration.
  provider    text        not null check (provider in ('anthropic')),
  -- Which credential family the sealed bytes hold. `api_key` is an
  -- `sk-ant-…` console key sent as `x-api-key`; `oauth_token` is the output of
  -- `claude setup-token`, sent as `Authorization: Bearer` with the
  -- `oauth-2025-04-20` beta. The Worker needs this to pick the auth header, and
  -- cannot read it out of the sealed blob without unsealing first.
  kind        text        not null check (kind in ('api_key', 'oauth_token')),
  -- Base64 of nonce || AES-256-GCM ciphertext. Opaque to Postgres and to the
  -- browser; only the Worker's key opens it.
  sealed      text        not null,
  -- Last four characters of the plaintext credential, for the settings UI to
  -- show "…AB12" so a user can tell two keys apart. Deliberately short enough
...
-- A user may only ever touch their own credentials. This is the same posture as
-- user_backups: the browser talks to PostgREST directly with the public anon
-- key, and RLS is the access control.
drop policy if exists own_llm_credential on public.llm_credentials;
create policy own_llm_credential on public.llm_credentials
  for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
...
-- Never published to realtime. Peer devices have no reason to receive a
-- credential row, and `user_backup_revisions` already carries the only CDC
-- signal the client subscribes to.
